...
public class TextInRectangle {
	...
	private final int starts[] = new int[MAX_LINES];
	private final int stops[] = new int[MAX_LINES];
	private int lines = 0;
	 ...
	 * 
	 * @param paint
	 *            - paint specifying the font
	 */
	public TextInRectangle(final Paint paint) {
	 ...
	 *            - top corner
	 */
	public void draw(final Canvas canvas, final int left, final int top) {
		if (textHeight == 0)
			return;
		...
		for (int n = 0; n <= lines; ++n) {
			...
			canvas.drawText(t, left, y, paint);
